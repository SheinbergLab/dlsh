...
namespace eval ::helpspec {
	...
	foreach a $args i [dl_tcllist [dl_fromto 0 [llength $args]]] {
		...
		switch -exact -- "$a" {
		    ...
		    -globnorm   { set  ::spec::psdVar(globnorm) [lindex $args [expr $i + 1]]}
		    default {puts "WARNING: $a is not a recognized option."}
		}
	    }
	}
	return 1
	...
	# 	%HELP%
	# 	::helpspec::plotSpectrogram args
	# 	WARNING: do not confuse with plotSpecgram, a similar function for use
	# 	with the lfp package.
	# 	This plot function is used to plot the output of ::spec::spectrogram
	# 	sgl is the list output by that function.
	# 	In position 0 are the power
	# 	In position 1 are the start times
	# 	In position 2 are the stop times for the data segments going
	# 	  into the analyses
	# 	In position 3 is a list of the frequencies for which the power
	# 	  was computed.
	#       sgl must be the first arguement
	#       second argument, optional, is a list of length two for the range of frequencies to plot
	# 	%END%
	
	set wl [lindex $args 0]
	if {[llength $args] == 2} {
	    set powrange [eval list [lindex $args 1]]
	} else {
	    set powrange [list [dl_min $wl:3] [dl_max $wl:3]]
	}
	dl_local freqs [dl_reverse $wl:3]
	#dl_view $freqs
	set stopidx [dl_max [dl_indices [dl_gte $freqs [lindex $powrange 0]]]]
	set startidx [dl_min [dl_indices [dl_lte $freqs [lindex $powrange 1]]]]
	puts "$startidx $stopidx"
	dl_local plotfreqs [dl_choose $freqs [dl_series $startidx $stopidx]]
	#dl_view $plotfreqs
	set p [dlp_newplot]
	dlp_setyrange $p [dl_min $plotfreqs] [dl_max $plotfreqs]
	dlp_setxrange $p [dl_min $wl:1] [dl_max $wl:2]
	dl_local tempdat [dl_llist]
	for {set d2pcnt 0} {$d2pcnt < [dl_length $wl:0]} {incr d2pcnt} {
	    dl_append $tempdat [dl_reverse $wl:0:$d2pcnt]
	}
	#dl_set tempdat $tempdat
	dl_local tempdat [dl_choose $tempdat [dl_llist [dl_series $startidx $stopidx]]]
	dl_local plotdat [dl_mult [dl_log10 [dl_collapse [dl_transpose $tempdat]]] 20.];#decibels
	#Normalization step, maybe better if not linear?
	set globmax [dl_max $plotdat]
	set globmin [dl_min $plotdat]
	dl_local plotdat [dl_char [dl_mult [dl_div [dl_sub $plotdat $globmin] [expr $globmax - $globmin]] 255]]
	dl_set $p:plotdat $plotdat
	puts [dl_length $tempdat]
	puts [dl_length ${tempdat}:0]
	dlp_winpostcmd $p "0 0 10 10" "dlg_image 5 5 \[dl_llist \"[dl_length $tempdat] [dl_length $tempdat:0]\" \[dl_collapse \[dl_combineLists %p:plotdat %p:plotdat %p:plotdat\]\]\] 10. 10. -center"
	return $p
    }


#####################################LFP Plotting Functions##################
#plotPSD and plotSpecgram are two special functions
#designed to be called from the lfp package for easy online
#plotting. I hope soon to deprecate these functions.
#Britt Anderson, 8/8/2007
	...
	dl_local freqs [dl_reverse [dl_mult [dl_fromto 0 [dl_length $sg:power:0:0]] $freq_res] ]
	#dl_view $freqs
	set stopidx [dl_max [dl_indices [dl_gte $freqs [lindex $::spec::psdVar(pow_range) 0]]]]
	set startidx [dl_min [dl_indices [dl_lte $freqs [expr double([lindex $::spec::psdVar(pow_range) 1])]]]]
	puts "$startidx $stopidx"
	dl_local plotfreqs [dl_choose $freqs [dl_series $startidx $stopidx]]
	#dl_view $plotfreqs
	set p [dlp_newplot]
	dlp_setyrange $p [dl_min $plotfreqs] [dl_max $plotfreqs]
	dlp_setxrange $p $::spec::psdVar(start) $::spec::psdVar(stop)
	dl_local tempdat [dl_llist]
	for {set d2pcnt 0} {$d2pcnt < [dl_length $sg:power]} {incr d2pcnt} {
	    dl_append $tempdat [dl_reverse [dl_meanList $sg:power:$d2pcnt]]
	}
	#dl_set tempdat $tempdat
	dl_local tempdat [dl_choose $tempdat [dl_llist [dl_series $startidx $stopidx]]]
	dl_local plotdat [dl_collapse [dl_transpose $tempdat]]
	dl_set $p:plotdat $plotdat
	dlp_winpostcmd $p "0 0 10 10" "dlg_image 5 5 \[dl_llist \"[dl_length $tempdat] [dl_length $tempdat:0]\" \[dl_collapse \[dl_combineLists %p:plotdat %p:plotdat %p:plotdat\]\]\] 10. 10. -center"
	return $p
    }
###########################################################
    proc chirp {tl fz k} {
	#produces a chirp signal. Imagined usual input to be a list of eeg times
