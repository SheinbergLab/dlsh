/*
 * tpool.cpp -- parallel Tcl script evaluation via thread pool
 *
 ...
 * infrastructure -- no application-specific registration, no event
 * dispatching, no network listeners.
 *
 * Follows the same pattern as the Tcl Thread package: each worker
 * creates a new interp, initializes it, and calls Tcl_InitStubs to
 * populate the stubs table.  The zipfs VFS mount is process-global,
 * so child interps see it automatically without calling TclZipfs_AppHook.
 *
 * Usage from Tcl:
 ...
 *
 * Creates a bare Tcl_Interp, initializes Tcl and the stubs table,
 * bootstraps the auto_path for dlsh packages, evals the combined
 * script, captures the result or error, and cleans up.
 *
 * The zipfs VFS is mounted process-wide by the host application,
 * so new interps see it without any additional setup.
 */
static void tpool_worker_func(tpool_worker_t *w)
{
    ...
    /*
     * Initialize stubs in the worker interp so all Tcl API calls
     * go through the stubs table (same pattern as Thread package).
     */
#ifdef USE_TCL_STUBS
    ...
    /* Bootstrap auto_path for dlsh packages from the zipfs VFS */
 ...
 */
static int tpool_map_command(ClientData, Tcl_Interp *interp,
                             int objc, Tcl_Obj *objv[])
{
    ...
    Tcl_Obj *dict = Tcl_NewDictObj();
    Tcl_DictObjPut(interp, dict,
        Tcl_NewStringObj("results", -1), results_list);
    Tcl_DictObjPut(interp, dict,
        Tcl_NewStringObj("missing", -1), Tcl_NewIntObj(missing));
    Tcl_DictObjPut(interp, dict,
        Tcl_NewStringObj("errors", -1), errors_list);
    Tcl_DictObjPut(interp, dict,
        Tcl_NewStringObj("n_threads", -1), Tcl_NewIntObj(num_threads));
...
#endif
extern "C" int Tpool_Init(Tcl_Interp *interp)
{
#ifdef USE_TCL_STUBS
    if (Tcl_InitStubs(interp, "8.6-", 0) == NULL) {
        return TCL_ERROR;
    ...
    if (Tcl_PkgProvide(interp, "tpool", "1.0") != TCL_OK) {
        return TCL_ERROR;
    }
